  ...
  computed: {
    totalCost() {
      ...
      for (let i = 0; i < this.cartStore.productList.length; i++) {
        cost +=
          Number(this.cartStore.productList[i].price) * Number(this.cartStore.productList[i].amount)
      }
      return cost
    },
  },
...
<template>
  ...
  <h3 v-if="totalCost === 0">Váš košík je prázdny :&#40;</h3>
  <div v-else>
  ...
  <v-snackbar v-model="snackbar" timeout="1000" location="top" color="error">
    Produkt bol odobraný z košíka
  </v-snackbar>
</template>
<style scoped>
ul {
